...
func NewBackgroundProcess(ctx context.Context, command string, opts ...ProcessOpt) *BackgroundProcess {
	...
	p.outPipe = bufio.NewReader(outPipe)
...
func (p *BackgroundProcess) SuperOut() {
	scanner := bufio.NewScanner(p.outPipe)
	scanner.Scan()
	fmt.Printf("Reading from subprocess: %s", scanner.Text())
}

func (p *BackgroundProcess) StdoutClear() {
	p.stdout.buf.Reset()
}
...
// Stdin пишем данные на вход
func (p *BackgroundProcess) Stdin(data string) {
	p.inPipe.Write([]byte("card\n"))
	//n, ok := io.Wr(p.inPipe, data)
	//if ok == nil {
